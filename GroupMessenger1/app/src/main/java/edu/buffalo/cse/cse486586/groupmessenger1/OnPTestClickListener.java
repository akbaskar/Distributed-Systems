    ...
    private boolean testQuery() {
        try {
            for (int i = 0; i < TEST_CNT; i++) {
                ...
                Log.i(TAG, "PTest return key: " + returnKey);
                Log.i(TAG, "PTest return value: " + returnValue);
                Log.i(TAG, "PTest original key: " + key);
                Log.i(TAG, "PTest original value: " + val);
                Log.i(TAG, "PTest original value length: " + val.length());
                Log.i(TAG, "PTest return value length: " + returnValue.length());

                if  (returnKey.equals(key)){
                    Log.i(TAG, "key values match ");
                }
                if  (returnValue.equals(val)){
                    Log.i(TAG, "val values match ");
                }
